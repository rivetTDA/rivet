...
class SimplexTree {
 ...
 //UPDATE THIS:       void add_simplex(std::vector<int> & vertices, int x, int y);	//adds a simplex (and its faces) to the SimplexTree; multi-grade is (x,y)
        ...
        MapMatrix* get_boundary_mx(std::vector<int>& face_order, std::vector<int>& coface_order);    //returns a boundary matrix for (hom_dim+1)-simplices with columns and rows a specified orders -- for vineyard-update algorithm
