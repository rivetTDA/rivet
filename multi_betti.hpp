...
MultiBetti::MultiBetti(SimplexTree* st, int dim)
{
	...
	for(int time = 1; time < num_times; time++)					//LOOP SHOULD START AT time = 0, RIGHT???
	{
		for(int dist = 1; dist < num_dists; dist++)				//LOOP SHOULD START AT dist = 0, RIGHT???
		{
			if(print_matrices) { std::cout << "  COMPUTING xi_0(" << "time" << "," << "dist" << ")\n"; }
	
			//build boundary matrix B+C
			MapMatrix* bdry_bc = (*bifiltration).get_boundary_mx(time, dist-1, dim);
			MapMatrix* bdry_c = (*bifiltration).get_boundary_mx(time-1, dist, dim);
			...
			//build matrix [B+C,D]
			MapMatrix* bcd = (*bifiltration).get_merge_mx(time, dist, dim);
			if(print_matrices)
			{
			...
			MapMatrix* bdry_d2 = (*bifiltration).get_boundary_mx(time, dist, dim+1);
			if(print_matrices)
			{
			...
			//compute dimension of homology at D
			MapMatrix* bdry_d1 = (*bifiltration).get_boundary_mx(time, dist, dim);
			if(print_matrices)
			{
			...
			if(print_matrices) { std::cout << "  COMPUTING xi_1(" << "time" << "," << "dist" << ")\n"; }
	
			//build boundary matrix A
			MapMatrix* bdry_a = (*bifiltration).get_boundary_mx(time-1, dist-1, dim);
			if(print_matrices)
			{
			...
			//build matrix [A,B+C]
			MapMatrix* abc = (*bifiltration).get_split_mx(time, dist, dim);
			if(print_matrices)
			{
			...
			MapMatrix* bdry_bc2 = (*bifiltration).get_boundary_mx(time, dist-1, dim+1);
			MapMatrix* bdry_c2 = (*bifiltration).get_boundary_mx(time-1, dist, dim+1);
