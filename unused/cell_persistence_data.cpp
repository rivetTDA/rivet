...
CellPersistenceData::CellPersistenceData(int v)
    : verbosity(v)
{
}
...
{
    if (verbosity >= 6) {
        std::cout << "    ---in this cell, x = " << x << " and y = " << y << "\n";
    }
    ...
    for (unsigned i = 0; i < xi.size(); i++) {
        ...
        if (projection.first == true) //then the projection exists
        {
            ...
            if (verbosity >= 8) {
                std::cout << "    ----support point " << i << " (" << xi[i].first << ", " << xi[i].second << ") projected to coordinate " << projection.second << "\n";
            }
        }
    }
    ...
    for (int i = 0; i < bifiltration->get_num_simplices(); i++) {
        ...
        if (sdata.dim == (dim - 1) || sdata.dim == dim || sdata.dim == (dim + 1)) {
            if (verbosity >= 8) {
                std::cout << "    --Simplex " << i << " has multi-index (" << sdata.x << ", " << sdata.y << "), dimension " << sdata.dim;
            }
            ...
            if (p_pair.first == true) //then projection exists
            {
                ...
                {
                    if (verbosity >= 8) {
                        std::cout << "; projected to " << p_pair.second << ", support point " << xi_it->second << "\n";
                    }
                ...
                {
                    if (verbosity >= 8) {
                        std::cout << "; projected to " << p_pair.second << ", which comes after all xi support points\n";
                    }
    ...
    for (std::multimap<int, int>::iterator it = simplex_p_order.begin(); it != simplex_p_order.end(); ++it) {
        if (verbosity >= 7) {
            std::cout << "CHECK: simplex ord ind " << ord_ind << ", order xi support pt ind " << it->first << ", global simp ind " << it->second << "\n";
        }
    ...
    for (std::multimap<int, int>::iterator it = face_p_order.begin(); it != face_p_order.end(); ++it) {
        if (verbosity >= 7) {
            std::cout << "CHECK: face ord ind " << ord_ind << ", order xi support pt ind " << it->first << ", global simp ind " << it->second << "\n";
        }
    ...
    if (verbosity >= 6) {
        std::cout << "    --Created boundary matrix for " << (dim + 1) << "-simplices\n";
    }
    if (verbosity >= 7) {
        bdry_coface->print();
    }
    ...
    if (verbosity >= 6) {
        std::cout << "    --Reduced boundary matrix for " << (dim + 1) << "-simplices\n";
    }
    if (verbosity >= 7) {
        bdry_coface->print();
    }
    ...
    for (int j = 0; j < bdry_coface->width(); j++) {
        ...
        if (i >= 0) {
            if (verbosity >= 7) {
                std::cout << "    ----pair (" << i << "," << j << ") ==>> (" << simplex_order_xi[i] << "," << coface_order_xi[j] << ")\n";
            }
    ...
    if (verbosity >= 6) {
        std::cout << "    --Found persistence pairs\n";
    }
    if (verbosity >= 7) {
        std::cout << "        ";
    ...
    for (unsigned i = 0; i < simplex_global.size(); i++) {
        ...
        {
            if (verbosity >= 7) {
                std::cout << "CHECK: simplex with global index " << simplex_global[i] << " is already paired\n";
            }
            ++poi_it;
        } else //simplex is not paired
        {
            if (verbosity >= 7) {
                std::cout << "CHECK: simplex with global index " << simplex_global[i] << " is NOT paired\n";
            }
    ...
    if (verbosity >= 6) {
        std::cout << "    --Created boundary matrix for " << dim << "-simplices\n";
    }
    if (verbosity >= 7) {
        bdry_simplex->print();
    }
    ...
    if (verbosity >= 6) {
        std::cout << "    --Reduced boundary matrix for " << dim << "-simplices\n";
    }
    if (verbosity >= 7) {
        bdry_simplex->print();
    }
    ...
    if (verbosity >= 6) {
        std::cout << "    --Found essential cycles\n";
    }
    if (verbosity >= 7) {
        std::cout << "        ";
